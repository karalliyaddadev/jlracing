...
export default function IntlVideoBannerAdminPage() {
  ...
  const fetchItems = useCallback(() => {
    setLoading(true);
    setError(null);
    apiFetch(`/api/video-banner?site=FOREIGN`)
      .then((r) => {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const upRes = await apiFetch(`/api/upload/foreign-video-banner`, {
        method: "POST",
        body: fd,
      });
      if (!upRes.ok) {
      ...
      <div className="video-admin__header">
        <div>
          ...
          <p className="video-admin__subtitle">
            The active video plays as a silent background loop on the
        ...
        <div className="video-admin__drop-area">
          {videoPreviewUrl ? (
            ...
            <label
              ...
              htmlFor="intl-video-banner-input"
            >
              <svg
                width="36"
                height="36"
